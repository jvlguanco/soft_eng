<div class="pt-8 pl-6 pr-20">
    <section class="container px-4 mx-auto">
        <div class="sm:flex sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-x-3">
                    <h2 class="text-lg font-medium text-gray-800">Submitted Application Forms</h2>

                    <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full">Total: {{ count($appFormList) }}</span>
                </div>
            </div>
        </div>
                    ...
                    <div class="overflow-hidden border border-gray-200 md:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 ">
                                @foreach ($appFormList as $applicant)
                                <tr>
        ...
        <div class="mt-6">
            {{ $appFormList->appends(['statusType' => $statusType, 'search' => $searchApplicant])->links() }}
        </div>

        <form action="{{ route('admin.generate.applicants.form', ['currentRoute' => $currentRoute]) }}" method="POST" target="__blank">
            @csrf
